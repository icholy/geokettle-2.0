...
public class GMLReader {
  ...
  public GMLReader(URL url) { 
      ...
      try {
    	  //this.url = new File( url.toString().substring(5) ).toURL();
  ...
  public void open() throws IOException, SAXException, XMLParsingException {
	...
	if((url.toString().substring(url.toString().length()-3,url.toString().length()).equalsIgnoreCase("gml")) || (url.toString().substring(url.toString().length()-3,url.toString().length()).equalsIgnoreCase("xml"))){
      ...
      try
      {
          ...
          for(int j = 0;j < props.length ; j++)
          {
            ...
            if(pt == java.sql.Types.VARCHAR) {
          	  ...
          	  value = new ValueMeta(props[j].getName().getLocalName(), ValueMetaInterface.TYPE_STRING);
          	  // TODO: check if there is a way to get max string length from AttributeType
            }
            else if(pt == java.sql.Types.INTEGER || pt == java.sql.Types.BIGINT) {
          	  // Integer
          	  debug = "integer attribute";
          	  value = new ValueMeta(props[j].getName().getLocalName(), ValueMetaInterface.TYPE_INTEGER);
            }
            else if(pt == java.sql.Types.DOUBLE) {
          	  // Double
          	  debug = "double attribute";
          	  value = new ValueMeta(props[j].getName().getLocalName(), ValueMetaInterface.TYPE_NUMBER);
            }
            else if(pt == java.sql.Types.DATE ) {
          	  // Date
          	  debug = "date attribute";
          	  value = new ValueMeta(props[j].getName().getLocalName(), ValueMetaInterface.TYPE_DATE);
            }
            else if( pt >= 10012 && pt <11019 )
            {
          	  // Geometry
          	  debug = "geometry attribute";
          	  value = new ValueMeta(props[j].getName().getLocalName(), ValueMetaInterface.TYPE_GEOMETRY);         	  
            }
  ...
  public Object[] getRow(Object[] r) throws KettleException{
    ...
    PropertyType[] props = ft.getProperties();
      ...
      {  
      	if (fc.size() == 0){
        	...
        	for(int j = 0; j < props.length; j++){
				...
				if(featProperties[j].getValue() == null) {
					...
					debug = "integer attribute";
					r[j] = new Long( ((Integer)featProperties[j].getValue()).longValue());					
				}
				...
				else if((pt >= 10012 && pt <=10016) || (pt >= 11012 && pt <=11019))
				{
					// Geometry
					debug = "geometry attribute";
					Geometry jts_geom = org.deegree.model.spatialschema.JTSAdapter.export((org.deegree.model.spatialschema.Geometry) featProperties[j].getValue()); 					
					r[j] = jts_geom;
				}
		...
		error = true;
		throw new KettleException("Unable to read row from Deegree datastore", e);
    }
    row_index--;
    return r;
  }
