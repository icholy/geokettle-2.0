...
public class RowDataUtil {
	...
	public static Object[] addRowData(Object[] one, int sourceLength, Object[] two) {
		// resizeArray() returns the existing rowData array if its size is >= than the 2nd param
		// thus result can be a reference to rowData instead of a new array;
		// is this the expected behavior?
		// Object[] result = resizeArray(one, sourceLength+two.length);
		Object[] result = createResizedCopy(one, sourceLength+two.length);
	...
	public static Object[] addValueData(Object[] rowData, int length, Object extra) {
		...
		Object[] result = createResizedCopy(rowData, length+1);
		result[length] = extra;
		return result;
	}
