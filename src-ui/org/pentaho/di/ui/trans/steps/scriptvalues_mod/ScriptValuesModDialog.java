...
public class ScriptValuesModDialog extends BaseStepDialog implements StepDialogInterface
{
    ...
    // -- BEGIN GeoKettle modification --
    private static Geometry DEFAULT_TEST_GEOMETRY = (new GeometryFactory()).createPoint(new Coordinate(0.0, 0.0));
    // -- END GeoKettle modification --
				...
				if (genMeta==null) {
				    ...
				    for (int i=0;i<rowMeta.size();i++)
				    {
				    	...
				    	case ValueMetaInterface.TYPE_GEOMETRY :
				    		string = valueMeta.getString(DEFAULT_TEST_GEOMETRY);
				    		break;
				    	// -- END GeoKettle modification --
				    	default: 
				    		break;
			...
			if (rowMeta!=null){
				...
				try{
				    ...
				    for (int i=0;i<rowMeta.size();i++)
				    {
                        ...
                        // -- Begin GeoKettle modification --
                        // TODO: encapsulate geometry construction in a utility class (part of row package)
                        if (valueMeta.isGeometry()) valueData = DEFAULT_TEST_GEOMETRY;
                        // -- End GeoKettle modification --
							...
							if (!varname.equalsIgnoreCase("row") && !varname.equalsIgnoreCase("trans_Status")){
								...
								if (result!=null){
									...
									// -- Begin GeoKettle modification --
					            		...
					            		// is it a JTS Geometry ? check the Java object type
										if(Context.jsToJava(result, Geometry.class) instanceof Geometry) {
											type=ValueMetaInterface.TYPE_GEOMETRY;
										}
									}
									// -- End GeoKettle modification --
								}
				...
				for (int i=0;i<r.size();i++){
						...
						if (wCompatible.getSelection()) {
							switch(v.getType()){
								...
								case ValueMetaInterface.TYPE_GEOMETRY: strItemToAdd=v.getName()+".getGeometry()"; break; //$NON-NLS-1$
