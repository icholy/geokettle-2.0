	...
	private synchronized Object[] metadataValues(RowMetaInterface rowMeta, Object[] rowData) throws KettleValueException
	{
		...
		for (int i=0;i<data.metanrs.length;i++)
		{
			...
			if (meta.getMeta()[i].getType()!=ValueMetaInterface.TYPE_NONE && fromMeta.getType()!=toMeta.getType())
            {
                ...
                // TODO: comment about PDI in general; this should be handled by ValueMetaInterface instead for
                // better reusability
                // (e.g. "public Object convertToType(Object obj, int valueMetaType)")
                case ValueMetaInterface.TYPE_GEOMETRY  : rowData[index] = fromMeta.getGeometry(rowData[index]); break;
