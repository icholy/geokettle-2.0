          ...
          switch (meta.getType()[i]) {
            ...
            case ValueMetaInterface.TYPE_GEOMETRY: {
            	if( classType.equalsIgnoreCase("org.mozilla.javascript.NativeJavaObject")
            			|| classType.equalsIgnoreCase("org.mozilla.javascript.Undefined") ) {
            		// is it a JTS Geometry ?
            		try {
            			Geometry g = (Geometry)Context.jsToJava(result, Geometry.class);
            			return g;
            		}
            			...
            			try {
            				...
            				ValueMeta vm = new ValueMeta("temp", ValueMetaInterface.TYPE_GEOMETRY);
            				return vm.getGeometry(s);
            			}
            			catch(EvaluatorException ee2) {
            				// a java Value class perhaps?
            				Value v = (Value)Context.jsToJava(result, Value.class);
            				if(v != null) {
            					return v.getGeometry();
            				}
            			}
            		}
