...
import org.pentaho.di.trans.steps.kmlfileinput.KMLFileInput;
...
//    private String filenameField;
     ...
     */
    public void setKmlFileName(String gisFileName)
    {
        this.kmlFileName = gisFileName;
    }
...
//            rep.saveStepAttribute(id_transformation, id_step, "accept_stepname", (acceptingStep!=null?acceptingStep.getName():"") ); //$NON-NLS-1$
			...
			String[] input, String[] output, RowMetaInterface info)
	{
        ...
        else
        {
            ...
            try
            {
            	gtr = new KMLReader(getURLfromFileName(transMeta.environmentSubstitute(kmlFileName)));
            	gtr.open();
                cr = new CheckResult(CheckResult.TYPE_RESULT_OK, Messages.getString("KMLFileInputMeta.Remark.FileExistsAndCanBeOpened"), stepMeta); //$NON-NLS-1$
                remarks.add(cr);
                
                RowMetaInterface r = gtr.getFields();
            
                cr = new CheckResult(CheckResult.TYPE_RESULT_OK, r.size()+Messages.getString("KMLFileInputMeta.Remark.OutputFieldsCouldBeDetermined"), stepMeta); //$NON-NLS-1$
                remarks.add(cr);
            }
            catch(KettleException ke)
            {
                cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, Messages.getString("KMLFileInputMeta.Remark.NoFieldsCouldBeFoundInFileBecauseOfError")+Const.CR+ke.getMessage(), stepMeta); //$NON-NLS-1$
                remarks.add(cr);
            }
            finally
            {
...
//	{
//		return new KMLFileInputDialog(shell, info, transMeta, name);
//	}
...
//    
    ...
    {//System.out.println("code:"+kmlFileName);
        return FileInputList.createFileList(space, new String[] { kmlFileName }, new String[] { null }, new String[] { "N" });
    }
...
//    }
    ...
    private java.net.URL getURLfromFileName(String filename) {
    	try {
    		...
    		// logError(Messages.getString("KMLFileInput.Log.Error.MalformedURL"));
    	}
    	return null;
    }
}
