...
//	    	}
	...
	private void openNextFile() throws KettleException
    {
        ...
        */
        ...
        try
        {
        	data.gtreader = new GeotoolsReader(data.file_gis.getURL());
        	data.gtreader.open();

        	logBasic(Messages.getString("GISFileInput.Log.OpenedGISFile")+" : ["+data.gtreader+"]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        {
            logError(Messages.getString("GISFileInput.Log.Error.CouldNotOpenGISFile1")+data.file_gis+Messages.getString("XBaseInput.Log.Error.CouldNotOpenXBaseFile2")+e.getMessage()); //$NON-NLS-1$ //$NON-NLS-2$
