...
//            rep.saveStepAttribute(id_transformation, id_step, "accept_stepname", (acceptingStep!=null?acceptingStep.getName():"") ); //$NON-NLS-1$
			...
			String[] Output, String[] output, RowMetaInterface info)
	{
        ...
        else
        {
            ...
            try
            {
            	gtr = new GeotoolsWriter(getURLfromFileName(transMeta.environmentSubstitute(gisFileName)));
            	gtr.open();
...
//            
//                cr = new CheckResult(CheckResult.TYPE_RESULT_OK, r.size()+Messages.getString("GISFileOutputtMeta.Remark.OutputFieldsCouldBeDetermined"), stepMeta); //$NON-NLS-1$
//                remarks.add(cr);
            }
