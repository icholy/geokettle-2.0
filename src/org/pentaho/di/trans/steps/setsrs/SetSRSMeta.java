...
public class SetSRSMeta extends BaseStepMeta implements StepMetaInterface  {
	...
	private void addRemarkOk(List<CheckResultInterface> remarks, String remark, StepMeta stepMeta) {
		CheckResult cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString(MESSAGE_CHECK+remark), stepMeta); //$NON-NLS-1$
		remarks.add(cr);
	}
	
	private void addRemarkOk(List<CheckResultInterface> remarks, String remark, String param, StepMeta stepMeta) {
		CheckResult cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK, Messages.getString(MESSAGE_CHECK+remark), param, stepMeta); //$NON-NLS-1$
		remarks.add(cr);
	}
	
	private void addRemarkError(List<CheckResultInterface> remarks, String remark, StepMeta stepMeta) {
		CheckResult cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR, Messages.getString(MESSAGE_CHECK+remark), stepMeta); //$NON-NLS-1$
		remarks.add(cr);
	}
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases,
		...
		try {
			...
			throw new KettleXMLException(Messages.getString(MESSAGE_EXC+"UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void readRep(Repository rep, long id_step,
		...
		try {
			...
			throw new KettleException(Messages.getString(MESSAGE_EXC+"UnexpectedErrorInReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, long id_transformation, long id_step) throws KettleException {
		try {
			...
			throw new KettleException(Messages.getString(MESSAGE_EXC+"UnableToSaveStepInfoToRepository")+id_step, e); //$NON-NLS-1$
		}
	}
