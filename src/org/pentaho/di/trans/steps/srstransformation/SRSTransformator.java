	...
	public Geometry transform(Geometry sourceGeometry) throws KettleStepException {
		...
		try {
			...
			try {
				...
				LogWriter.getInstance().logDetailed("GeoKettle", "Warning: using lenient datum shift (without Bursa Wolf parameters for SRS transform"); 
		...
		LogWriter.getInstance().logDetailed("GeoKettle", "Spatial transformation succeeded from "+ sourceGeometry.getSRID()+" to "+targetGeometry.getSRID());
